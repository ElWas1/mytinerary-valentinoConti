...
const CommentInput = ({ itineraryId, userId }) => {
    ...
    const handlePostComment = () => {
        const object = {
            comment: commentInputContent.current.value,
            id: itineraryId,
            userId: userId
        }
        ...
        <>
            <div ref={inputDiv} className="flex flex-row bg-black outline outline-1 outline-green-500 rounded-xl mt-16 w-full p-2">
                {user ?
                    ...
                    : <input disabled={true} ref={commentInputContent} className="bg-black w-full p-1 rounded-xl outline-none" name="comment" id="comment" type="text" placeholder="Sign In to post comments." />}
