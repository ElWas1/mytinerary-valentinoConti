...
const UserProfile = () => {
  ...
  const handleShowOptions = () => {
    if (user?.email === visitedUser.email && user?.username === visitedUser.username) {
      (form.current.className === 'hidden') ? form.current.className = 'my-12 flex flex-col gap-4' : form.current.className = 'hidden'
    }
  }
  ...
  return (
    <div className='flex flex-col items-center gap-4 min-h-full my-8'>
      <div className='relative'>
        ...
        <button className=''>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="absolute rounded-3xl outline outline-1 outline-white bg-black right-0 bottom-0 p-3 w-12 h-12 hover:bg-gray-800 duration-300">
